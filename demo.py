...
from utils.visualize import vis_mask, vis_octagon, vis_ex, vis_class, vis_bbox,feature
from dextr import Dextr

torch.backends.cudnn.benchmark = False
os.environ["CUDA_VISIBLE_DEVICES"] = '1'

class_name = ['__background__','Nucleus']
...
def parse_args():
    ...
    parser.add_argument("--model_path",
                        default='./cache/nnet/ExtremeNet_10000(babe).pkl')
    parser.add_argument("--show_mask", action='store_true',default="show_mask",
...
if __name__ == "__main__":
    ...
    cluster_radius= configs["db"]["cluster_radius"]
    ...
    for image_id, image_name in enumerate(image_names):
        ...
        if suppres_ghost:
            for j in range(1, categories + 1):
                ...
                for k in range(n):
                    inside_score = 0
                    if top_bboxes[image_id][j][k, 4] > 0.5:
                        for t in range(n):
